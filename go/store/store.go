...
func Open() (*Store, error) {
	client, err := ent.Open(
		config.Get(config.DatabaseDriverNameKey),
		config.Get(config.DatabaseFileKey))
	if err != nil {
...
func (store *Store) createThing(ctx context.Context, name string) error {
	...
	log.Printf("created thing: %s", name)
	return nil
}
...
func (store *Store) updateThing(ctx context.Context, t *ent.Thing) error {
	...
	log.Printf("updated thing: %s", t.Name)
	return nil
}
